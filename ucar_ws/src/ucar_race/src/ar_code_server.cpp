...
//    double k3 = 0;
    ...
    int width = 640;
    int height = 480;
    int x_offset = static_cast<int>(width * 0.25);
    int y_offset = static_cast<int>(height * 0.25);
...
bool ARCodeNode::ez_cmd_move(Eigen::Vector3d position,Eigen::Vector3d euler_angle){
    ...
    while (ros::ok()&&!move_finish)
    {
        ...
        ld -=  sqrt(current_odom_.twist.twist.linear.x*current_odom_.twist.twist.linear.x + current_odom_.twist.twist.linear.y*current_odom_.twist.twist.linear.y)*delta_t;
...
bool ARCodeNode::getGoalPose(Eigen::Vector3d & T_goal_in_base, Eigen::Vector3d & Angle_goal_in_base){
    ...
    while (inputVideo.grab()&&ros::ok()){
        ...
        inputVideo >> image;//抓取视频中的一张照片
        flip(image,imageFlip,1);//1代表水平方向旋转180度
        imageFlip.copyTo(imageCopy);
...
//                continue;
        ...
        std::vector<cv::Vec3d> rvecs, tvecs; //旋转矩阵rvecs和平移矩阵tvecs
        cv::aruco::estimatePoseSingleMarkers(corners, marker_size, cameraMatrix, distCoeffs, rvecs, tvecs);//求解Pose
        int id_id = 0;
...
//        }
        ...
        //mark_pose_in_camera
        Eigen::Vector3d T_mark_in_cam = Eigen::Vector3d(tvecs[id_id][0]/1000.0, tvecs[id_id][1]/1000.0, tvecs[id_id][2]/1000.0); //camera frame 下 marker的 平移
        Eigen::Quaterniond Q_mark_in_cam(matrix3); //camera frame 下 marker的 旋转
        // tf::Quaternion quat(Q_mark_in_cam.x(),Q_mark_in_cam.y(),Q_mark_in_cam.z(),Q_mark_in_cam.w());

        //goal_pose_in_mark
        Eigen::Vector3d T_goal_in_mark = Eigen::Vector3d(0.0, d_y_, d_x_); //mark frame 下 goal的 平移
        Eigen::Quaterniond Q_goal_in_mark =                          //mark frame 下 goal的 旋转
                Eigen::AngleAxisd(0.0, Eigen::Vector3d::UnitZ()) *
                Eigen::AngleAxisd( 3.14159/2, Eigen::Vector3d::UnitY()) *
                Eigen::AngleAxisd(-3.14159/2, Eigen::Vector3d::UnitX());

        //camera_pose_in_base
        Eigen::Vector3d    T_camera_in_base = Eigen::Vector3d(0.15, 0.0, 0.15); //mark frame 下 goal的 平移
        Eigen::Quaterniond Q_camera_in_base =                          //mark frame 下 goal的 旋转
        ...
        tf::Matrix3x3(quat_goal_in_base).getRPY(roll, pitch, yaw);//进行转换
